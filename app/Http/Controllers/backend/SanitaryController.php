...
{
    public function sanitaryProfile  (Request  $request){
        try{
            ...
            $washing = DB::table('various_servicing_order')
                ...
                ->where('b.status', 1)
                ->where('b.id', $id)
                ->where('various_servicing_order.type', 'স্যানিটারি')
                ->orderBy('various_servicing_order.id','desc')
                ->paginate(20);
            return view('backend.sanitaryProfile ',['washings' =>$washing,'users'=> $users]);
        }
