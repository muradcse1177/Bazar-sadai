...
class RoomCleanerController extends Controller
{
    public function roomCleanerProfile  (Request  $request){
        try{
            ...
            $washing = DB::table('cleaning_order')
                ...
                ->where('b.status', 1)
                ->where('b.id',$id )
                ->where('cleaning_order.type','রুম' )
                ->orWhere('cleaning_order.type','ওয়াশরুম' )
                ->orderBy('cleaning_order.id','desc')
                ->paginate(20);
            return view('backend.roomCleanerProfile ',['washings' =>$washing,'users'=> $users]);
        }
