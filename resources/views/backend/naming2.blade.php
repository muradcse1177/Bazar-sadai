...
@section('title',' নেমিং ২')
@section('page_header', ' নেমিং ২ ব্যবস্থাপনা')
@section('mainLiAdd','active menu-open')
@section('naming2','active')
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                ...
                <div class="divform" style="display:none">
                    {{ Form::open(array('url' => 'insertNaming2',  'method' => 'post')) }}
                    {{ csrf_field() }}
                    <div class="box-body">
                        <div class="form-group">
                            <label>দেশের নাম</label>
                            <select class="form-control select2 div_name" name="divId" style="width: 100%;" required>
                                <option selected>দেশ  নির্বাচন করুন</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for=""> নেমিং ২</label>
                            <input type="text" class="form-control name2" id="name2"  name="name2" placeholder="নেমিং ২ লিখুন" required>
                        </div>
                        <div class="form-group">
                            <label for=""> নাম</label>
        ...
        <div class="col-md-12">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">নেমিং -২   লিস্ট </h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body table-responsive">
                    <table class="table table-bordered">
                        ...
                        @foreach($naming2s as $naming2)
                            <tr>
                                <td> {{$naming2-> divName}} </td>
                                <td> {{$naming2->naming2}} </td>
                                <td> {{$naming2->n_name}} </td>
                                <td class="td-actions text-center">
                                    <button type="button" rel="tooltip" class="btn btn-success edit" data-id="{{$naming2->n_id}}">
                                        <i class="fa fa-edit"></i>
                                    </button>
                                    <button type="button" rel="tooltip"  class="btn btn-danger delete" data-id="{{$naming2->n_id}}">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    {{ Form::open(array('url' => 'deleteNaming2',  'method' => 'post')) }}
                                    {{ csrf_field() }}
        ...
        function getRow(id){
            $.ajax({
                ...
                success: function(response){
                    var data = response.data;
                    $('.name').val(data.name);
                    $('.div_name').val(data.div_id);
                    $('.name2').val(data.naming2);
                    $('.id').val(data.id);
                    $('.select2').select2()
                }
            });
